...
class GA:
    ...
    def solve_maxOne(self):
        while True:
            self.selectParents()
            self.generateChildren(0.8)
            self.mutateChildren(0.03)
            self.iterations += 1
            # print("Population:", self.population)

            for individual in self.population:
                print("Individual:", individual, "Fitness:", self.population[individual][1])

            print("Total Fitness:", self.totalFitness)
            print("Itterations: ", self.iterations)
            found = False
            for individual in self.population:
                if self.population[individual][1] == self.individualSize:
                    found = True
                    break
            if found:
                break
